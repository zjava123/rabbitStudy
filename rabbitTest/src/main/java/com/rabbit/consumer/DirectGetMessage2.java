...
public class DirectGetMessage2 {
	
	private static final String QUEUE_MESSAGE = "路由器模式";
	
	private static final String KEY_MESSAGE = "路由器模式2";
	...
	public static void directConsumer() throws IOException, ShutdownSignalException, ConsumerCancelledException, InterruptedException{
		...
		//绑定队列到交换机
		channel.queueBind(QUEUE_MESSAGE, DIRECT_NAME, "create");
		channel.queueBind(KEY_MESSAGE, DIRECT_NAME, "insert");
		...
		while (true) {
			...
			System.out.println("路由器模式2获取的消息为:"+message);
			Thread.sleep(1000);
